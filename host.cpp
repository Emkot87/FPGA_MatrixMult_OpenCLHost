  ...
  // Create the test data
	...
	for(int i = 0 ; i < DATA_SIZE ; i++){
		for(int j = 0 ; j < DATA_SIZE ; j++){
			source_in1[i * DATA_SIZE + j] = rand() % 255;
			//std::cout << in1[i][j] << " ";
		}
		//printf("\n");
	}
	...
	for(int i = 0 ; i < DATA_SIZE ; i++){
		for(int j = 0 ; j < DATA_SIZE ; j++){
			source_in2[i * DATA_SIZE + j] = rand() % 255;
			//std::cout << in2[i][j] << " ";
		}
		//printf("\n");
	}
  ...
  for (int i = 0; i < DATA_SIZE; i++) {
    for(int j = 0; j < DATA_SIZE; j++){
        ...
        for(int k = 0; k < DATA_SIZE; k++){
            result += source_in1[i *DATA_SIZE + k] * source_in2[k *DATA_SIZE + j];
        }
        source_sw_results[i *DATA_SIZE + j] = result;
        source_hw_results[i *DATA_SIZE + j] = 0;
    }
